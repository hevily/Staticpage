(function ($) {
    dk_slideplayer = function (object, config) {
        ...
        this.factory = function () {
            //元素定位
            $(this.obj).css({ position: "relative", zIndex: "0", margin: "0", padding: "0", width: this.config.width, height: this.config.height })
            $(this.obj).prepend("<div style='position:absolute;z-index:20;' class='dkNum'></div>");
            $(this.obj + " li").css({ position: "absolute", top: "0", left: "0", width: "100%", height: "100%", overflow: "hidden" }).each(function (i) {
                if (config.fontsize == "0") {
                    $(_this.obj + " div").append("<a tabindex='" + i + "'><img src='" + $(object).find("li img").attr("src") + "' /></a>");
                }
                else {
                    $(_this.obj + " div").append("<a tabindex='" + i + "'></a>");
                }
            });
            ...
            $(this.obj + " .dkTitleBg").css({ position: "absolute", margin: "0", padding: "0", zIndex: "1", bottom: "0", left: "0", width: "100%", background: "#000", opacity: "0.4", overflow: "hidden" })
            //插入标题
            $(this.obj).prepend("<div class='dkTitle'></div>");
            $(this.obj + " p").each(function (i) {
                $(this).appendTo($(_this.obj + " .dkTitle")).css({ position: "absolute", margin: "0", padding: "0", zIndex: "2", width: "100%", textDecoration: "none", background: "none", opacity: "1", overflow: "hidden" });
                if (i != 0) { $(this).hide() }
            });
            this.slide();
            this.addhover();
        ...
        this.resetclass = function (obj, i) {
            $(obj).css({ float: "left", cursor: "pointer" });
            $(obj).removeClass("cur");
            $(obj).eq(i).addClass("cur");//({ color: "#FFFFFF", background: "#FF7D01", textDecoration: "none" });
        }
        this.factory();
    }
})(jQuery)